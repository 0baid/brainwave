...
const Section = ({className,id,crosses,crossesOffset,customPadding,children}) => {
  return (
    <div id={id} className={`relative ${customPadding || `py-10 lg:py-16 xl:py-20 ${crosses ? 'lg:py-32 xl:py-40' : ''} ${className || ''}` }`}>
        {children}
        <div className='hidden absolute w-0.25 h-full bg-stroke-1 pointer-events-none md:block lg:left-7.5 xl:left-10 top-0 left-5'/>
        <div className='hidden absolute w-0.25 h-full bg-stroke-1 pointer-events-none md:block lg:right-7.5 xl:right-10 top-0 right-5'/>
        {crosses && (
            <>
